...
public class PipeconfLoader {
    ...
    public static final String PIPELINE_APP_NAME = "org.onosproject.pipelines.aggregator";
    ...
    private static DefaultPiPipeconf.Builder bmv2Pipeconf(
        ...
        return basePipeconfBuilder(BMV2, profile, platform, p4InfoUrl, cpuPortUrl)
                .addExtension(ExtensionType.BMV2_JSON, bmv2JsonUrl);
    }


    private static DefaultPiPipeconf.Builder tofinoPipeconf(String profile, String platform)
            throws FileNotFoundException {
        ...
        return basePipeconfBuilder(TOFINO, profile, platform, p4InfoUrl, cpuPortUrl)
    ...
    private static DefaultPiPipeconf.Builder basePipeconfBuilder(
            String target, String profile, String platform, URL p4InfoUrl, URL cpuPortUrl) {
        final String pipeconfId = platform.equals(DEFAULT_PLATFORM)
                // Omit platform if default, e.g. with BMv2 pipeconf
                ? format("%s.%s.%s", BASE_PIPECONF_ID, profile, target)
                : format("%s.%s.%s.%s", BASE_PIPECONF_ID, profile, platform, target);
        final PiPipelineModel model = parseP4Info(p4InfoUrl);
        return DefaultPiPipeconf.builder()
                .withId(new PiPipeconfId(pipeconfId))
                .withPipelineModel(model)
                .addBehaviour(PiPipelineInterpreter.class, AggregatorInterpreter.class)
                .addBehaviour(Pipeliner.class, AggregatorPipeliner.class)
